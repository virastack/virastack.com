...
export async function PasswordExampleDetailDoc({ id }: PasswordExampleDetailDocProps) {
  ...
  return (
    <>
      ...
      <DocsProse>
        <p>
          <Link href="/password/docs/examples">← Tüm örnekler</Link>
          <span className="mx-2 text-border">·</span>
          <code>{example.variant}</code>
        </p>

        <h2 id="onizleme">Önizleme</h2>
        ...
        <p>
          Hook API için <Link href="/password/docs/use-vira-password">useViraPassword()</Link>, stil
          ve ikonlar için <Link href="/password/docs/customization">customization</Link>.
        </p>
      </DocsProse>
    </>
  );
